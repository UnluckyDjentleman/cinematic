import {Button, ComboboxItem, Flex} from "@mantine/core";
...
import { setMovieFilter, setPage } from "../../store/reducers/movieListFilterReducer";
...
export default function Header(){
    ...
    const genreSetted=useRef<string|null>(movieGenresArray.movieGenres.find(el=>el.id===movieFilter.genre)?.name as string);


    const onFilterSubmit=useCallback(()=>{
        console.log(movieGenresArray.movieGenres.find(el=>el.name===genreSetted.current))
        dispatch(
            setMovieFilter({
                page: 1,
                genre: movieGenresArray.movieGenres.find(el=>el.name===genreSetted.current)?.id,
    ...
    const onPickGenre=useCallback((value: string|null)=>{
        genreSetted.current=value as string
        onFilterSubmit()
    },[genreSetted])

    const onRatingLte=useCallback((value: number|string)=>{
        movieVoteLte.current=value as number;
        onFilterSubmit()
    },[movieVoteLte])

    const onRatingGte=useCallback((value: number|string)=>{
        movieVoteGte.current=value as number;
        onFilterSubmit()
    },[movieVoteGte])

    const onPickYear=useCallback((value: string|null)=>{
        movieYearRelease.current=parseInt(value as string);
        onFilterSubmit()
    },[movieYearRelease])

    const onPickSortBy=useCallback((value: string|null)=>{
    ...
    return (
        <div style={{margin: "3%"}}>
            <Flex mt="md" mb="xs" justify="space-between">
                <Selector selector={movieGenresArray.movieGenres.map(el=>el.name)} label="Genre" placeholder="Select Genre" onPickVal={onPickGenre} value={genreSetted.current as string}></Selector>
                <VoteInput placeholder="From" label="Ratings" onVote={onRatingGte} value={movieVoteGte.current}></VoteInput>
                <VoteInput placeholder="To" label={undefined} onVote={onRatingLte} value={movieVoteLte.current}></VoteInput>
                <Selector selector={listYear.map(String)} label="Year" placeholder="Year" onPickVal={onPickYear} value={movieYearRelease.current?.toString()}></Selector>
                <Button variant="transparent" style={{ alignSelf: "flex-end", marginBottom: "0" }} color="#7B7C88" onClick={onFilterReset}>Reset Filters</Button>
            </Flex>
            <Flex mt="md" mb="xs" justify="flex-end">
                <Selector selector={sortByArray} label="Sort By" placeholder="Sort By" onPickVal={onPickSortBy} value={movieSortBy.current as string}></Selector>
            </Flex>
        </div>
    )
}