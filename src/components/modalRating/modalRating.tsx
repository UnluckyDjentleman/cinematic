...
export default function ModalRate({opened, close, onChangeRating}:{opened: boolean, close: ()=>void, onChangeRating: (value: number|undefined)=>void}){
    ...
    const ratedMovie=useIsRated(user as User, movie.movie?.movieSearchInfo as MovieSearchInfo)

    console.log(ratedMovie);

    const [rating, setRating] = useState<number|undefined>(ratedMovie.isRated?.rating);

    const setRatingValue=useCallback((ratingValue: number)=>{
        console.log(ratingValue);
        setRating(ratingValue);
    },[rating])

    const onChangeRatingValue=useCallback(async (rating: number|undefined)=>{
        setRating(rating)
        ...
        <>
            <Modal opened={opened} onClose={close} title="Rating" >
                <Flex direction="column">
                    ...
                    w="54%"
                    mt="lg">
                        <Button size="xs" radius={8} color="#9854F6" onClick={()=>onChangeRatingValue(rating)}>
                            Save
                        </Button>
                        <Button
                        size="xs"
