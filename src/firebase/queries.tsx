...
async function isMovieRated(user: User, movie: MovieSearchInfo) {
  ...
  if (!docSnap.empty) {
    rated = docSnap.docs
      .map((el) => el.data())
      .map((el) => el.rate)[0] as number;
  }
  return { isRated: !docSnap.empty, rating: rated };
}

async function addMovieRating(
  user: User,
  ...
  if (!docSnap.empty) {
    await updateDoc(
      doc(firebaseData.database, "LikedMovies", docSnap.docs[0].id),
      { rate: rating.toString() }
    );
  } else {
    await addDoc(firebaseData.likedMoviesCollection, {
      userid: user.email,
...
async function deleteMovie(user: User, movieid: number | undefined) {
  ...
  if (!docSnap.empty) {
    await deleteDoc(
      doc(firebaseData.likedMoviesCollection, docSnap.docs[0].id)
    );
  }
}
